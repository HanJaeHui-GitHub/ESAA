...
와!!!!!!! 하루종일 3명이서 나눠서 하다가 회의하면서 잠깐 5분 돌렸는데 레전드 조합 탄생!!!! 내가 해냄!!!!!
...
수가 클수록 고위험도 가능성이 커지는 변수(+)와 수가 클수록 고위험도 가능성이 작아지는 변수(-)를 구분하는게 제일 중요! : **반전 처리** 부분에서 진행

<img width="557" height="383" alt="image" src="https://github.com/user-attachments/assets/0093c3e8-4a19-4f01-82a9-4f257cd09c6a" />

- 인자 1 : 고령화 및 독거노인 특성

  고령화율, 고령자밀집도지수, 독거노인_비율 (값이 클수록 안 좋은 **부정적**인 지표)

  **인자점수 값이 클수록** 해당 지역의 고령화율과 독거노인 비율이 높음 =  **노인 취약도가 높음**

- 인자 2 : 경제 활동 및 소비 여력

  월소득평균, 월신용평점평균, 월카드총이용금액평균 (값이 클수록 좋은 **긍정적**인 지표)

  **> 스케일링에서 반전 처리 진행**

  **인자점수 값이 클수록** 월소득, 신용평점, 카드 이용 금액이 낮음 = **경제적 여력 취약**

- 인자 3 : 인지 건강 서비스 지원

  노인1천명당_여가복지시설수_노인교실, 노인1천명당_치매안심센터수 (값이 클수록 좋은 **긍정적**인 지표)

  **> 스케일링에서 반전 처리 진행**

  **인자점수 값이 클수록** 인지 건강 관련 시설 수 부족 = **노인 취약도 높음**

- 인자 4 : 금융 취약도 및 부채 부담

  금융취약도지수, 월대출평균잔액 (값이 클수록 안 좋은 **부정적**인 지표)

  **인자점수 값이 클수록** 금융 취약도가 높고 대출 부담이 큼 = **경제적으로 취약**

- 인자 5 : 취약 노인 생활 인프라

  국민기초생활보장_독거노인_수급자비율 (값이 클수록 안 좋은 **부정적**인 지표)

  노인1천명당_재가복지시설수, 노인1천명당_무료급식소수, 노인1천명당_자립형일자리수행기관수 (값이 클수록 좋은 **긍정적**인 지표)

  **> 스케일링에서 반전 처리 진행**

  **인자점수 값이 클수록** 생활 인프라가 부족하고 생활보장 수급자 비율 높음 = **생활 인프라 취약**

인자 분석 결과를 이용해 행정동별로 인자 점수, 점수(인자 점수 단순 합), 점수(0~100) 계산

**이때 점수는 클수록 고위험에 가까움을 의미**

### 공간 자기상관성 분석

산출한 점수를 이용해 공간 자기상관성 분석 진행

**WHY?** 복지 사각지대 고위험 지역을 클러스터링하여 분별하기 위해

**HOW?** Moran's I(지리적으로 가까운 지역들 간에 유사한 값들이 얼마나 공간적으로 클러스터 되어 있는지 수치로 나타내는 지표) 사용

<img width="76" height="91" alt="image" src="https://github.com/user-attachments/assets/29fb11a2-e0f3-457f-af49-420edbc5d6e4" />

- 3: LL (Low-Low) 대상 지점의 값이 낮고, 주변 이웃들도 값이 낮음 저값이 모여 있는 ‘콜드스팟’ (Cold Spot)
  
- **1: HH (High-High) 대상 지점의 값이 높고, 주변 이웃들도 값이 높음 고값이 모여 있는 ‘핫스팟’ (Hot Spot)** $\rightarrow$ 위험군으로 선정
...
위험군 데이터의 [인자 점수, 점수, 점수(0~100])를 이용해 k=3(저,중,고)로 클러스터링 진행

cluster|인자1|인자2|인자3|인자4|인자5|점수|점수(0~100)
-|-|-|-|-|-|-|-
0|1.755929|-0.489304|-0.029140|-0.825806|0.478377|0.279001|77.068600 $\rightarrow$ 중위험
...
- 클러스터 2 (고위험군) - 병천면, 수신면, 성남면

  인자 1(고령화 및 독거노인 특성), 인자 4(금융 취약도 및 부채 부담), 인자 5(취약 노인 생활 인프라) 점수 높음

  > 고령화가 심화되고 독거 노인 비율이 높으며 경제적 어려움과 생활 인프라 부족이 복합적으로 작용하여 사회적·경제적으로 가장 취약한 노인 집단이 있는 지역

- 클러스터 0 (중위험군) - 동면, 북면

  인자 1(고령화 및 독거노인 특성), 인자 5(취약 노인 생활 인프라) 점수 높음

  인자 2(경제활동 및 소비 여력 인자 점수), 인자 4(금융 취약도 및 부채 부담) 점수 낮음

  > 노인 비중과 독거 노인 비율이 높지만 경제적/금융적 여력은 상대적으로 양호한 지역(고위험군과의 가장 큰 차이점), 그러나 일부 생활 인프라 부족 문제가 존재하기 때문에 잠재적 취약성에 대비할 필요성이 보임

- 클러스터 1 (저위험군) - 원성 1,2동, 중앙동, 문성동, 성정 1,2동

  인자 2(경제활동 및 소비 여력 인자 점수) 점수 높음

  인자 3(인지 건강 서비스 지원), 인자 5(취약 노인 생활 인프라) 점수 낮음

  >  노인 인구 비중은 평균 수준이며 복지 인프라는 비교적 잘 갖춰진 지역, 그러나 월소득 신용 평점 등이 낮아 경제활동 및 소비 여력이 취약하여 잠재적 위험에 노출될 수 있는 지역
...
참고 자료 : [천안시청 - 2025년 천안시 노인복지 시행 계획](https://call.cheonan.go.kr/cmm/fms/FileDown.do?atchFileId=FILE_000000000776127&fileSn=0)

2025년 추진 계획의 5개 과제를 위험군별 특성에 따라 우선 배정하여 단계적으로 추진하는 방안 제시

은주언니의 '복지사각지대에 대한 단계별 대응 필요 : 발굴 $\rightarrow$ 지원 $\rightarrow$ 관리' 개념 중 '지원' 단계에서 준비해 온 정책 제안 내용 사용하기로 채택됨!!!!!

### 피피티 담당 파트 : 데이터 가공 & 세부지표 구축 & 데이터 병합

#### 데이터 가공

천안시의 노인 복지와 관련된 다양한 데이터를 가능한 많이 수집한 뒤, 서로 다른 형식을 하나의 양식으로 통일하여 가공

실제 행정 서비스는 행정동 기준으로 운영됨

특히 이 프로젝트의 목적이 행정 편의에 기반한 정책 제안이기 때문에 행정동 단위로 데이터를 맞추는 것이 타당하다고 판단

$\hookrightarrow$ 구청 사이트를 크롤링하여 기준 행정동 데이터 파일을 생성하고, 이를 기반으로 매핑 과정을 거쳐 데이터 가공 진행
...
6. [경제] 시계열 지표
7. [시설] 복지 시설 지수
8. [시설] 노인 주거보장 및 건강보장 시설
9. [시설] 노인 사회서비스 제공 및 소득보장 시설
10. [교통] 교통 취약 노인 관련

#### 데이터 병합

분석의 가능성을 최대한 넓히기 위해 다양한 세부지표 구축

$\hookrightarrow$ **다양한 조합을 실험하여 최적의 분석 결과를 얻기 위함**

모든 지표를 사용하기보다 **인자분석 모델의 품질과 해석력을 높이기 위해** 변수 조합을 여러 관점에서 검토

[변수 조합 선정 기준]

1. 통계적 기준

   VIF 분석 & 변수 간 상관관계 히트맵 $\rightarrow$ 과도하게 상관된 변수 사전 확인

2. 분석 적합성 기준

   인자분석 적합성 검정(KMO, Barrtlett 구형성 검정) $\rightarrow$ 통계적으로 타당한 조합 확인

3. 의미적 기준

   적합성 검정을 통과한 조합 중 인자 내 유사 지표들이 가장 명확하게 묶이는 경우 최종 선택 $\rightarrow$ 모델의 해석력 강화

$\hookrightarrow$ 모델 적합성과 해석력이 가장 우수한 14개 지표 선정

$\therefore$ 최종 데이터 : (31개 행정동) x (14개 세부 지표)
...
1. 천안시 노인 복지 사각지대 지도
   ...
   천안시의 노인 복지 사각지대 위험군 정보 지도 시각화

   고위험군 - 빨강, 중위험군 - 주황, 저위험군 - 초록, 비위험군 - 베이지로 구분하여 표시

   지도를 클릭하여 세부 행정동별 세부 정보 확인 가능

2. 인자 점수

   5가지 인자를 꼭짓점으로 갖는 방사형 차트를 활용하여 시각화

   $\hookrightarrow$ 차트 면적 = 해당 지역의 인자 분석을 통해 산출된 위험 점수(0~100)

   각 인자의 기여 비율(가중치)은 막대 그래프로 표현

3. 분석 결과 요약 정리

   지역적 공간 자기상관 분석 결과와 인자 분석 최종 점수를 텍스트로 제공

   $\rightarrow$ 해당 행정동의 노인 복지 사각지대 여부와 위험 수준(고,중,저) 확인 가능
